    ...
    def __evaluateSwarm(self):
        ...
        if self.UseParallel:
            ...
            self.CurrentSwarmFitness[:] = Parallel(n_jobs=nCores)( 
                    delayed(self.ObjectiveFcn)(self.Swarm[i,:],self.stock,self.order) for i in range(nParticles) )
        else:
            self.CurrentSwarmFitness[:] = [self.ObjectiveFcn(self.Swarm[i,:],self.stock,self.order) for i in range(nParticles)]
    ...
    def optimize( self ):
        ...
        while not doStop:
            ...
            for p in range(nParticles):
                ...
                self.Velocity[p,posInvalid] = 0.0 #mallon den xreiazete
